...
(1-pt(qt(1-alpha/2,df)-dt,df) + pt(qt(alpha/2,df)-dt,df))*100
```
We can check this by running the code in the function pwr.t.test.
```{r, eval = FALSE}
pwr.t.test(n=20, d=1, type="two.sample", alternative="two.sided", sig.level=alpha)[[4]]*100
```
The probability of detecting 1 sd difference with only 20 animals per group is 86.89%
...
```{r}
test <- chisq.test(table(dirbiliB))
test
test$p.value
```
...
The wilcox is a useful approach as it does not assume anything about the distribution or parameters. As our observations consist of only __three__ values it is difficult to assume a continuous distribution. Wilcox works around this issue and calculates a p-value similarly low as the chi square test and much lower than our 5% cutoff.
...
The wilcox is a useful approach as it does not assume anything about the distribution or parameters. As our observations consist of only two values it is difficult to assume a continuous distribution. Wilcox works around this issue and calculates a p-value equal to the chi square test of 1.

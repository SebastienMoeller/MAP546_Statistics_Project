...
pacman::p_load(equivalence, tidyverse, ggplot2, dplyr, plyr)
```
...
For this reason, this test is often used as an alternative to t test's whenever the population cannot be assumed to be normally distributed. Even if the normality assumption holds, it has been shown that the efficiency of this test compared to t-test is almost 95%.
```{r}
...
# Calculate the p-value
tost((calciumC %>% filter(regimen == 'control'))[,2], (calciumC %>% filter(regimen == 'MON810'))[,2], alpha = 0.05, epsilon = 2*sd(calciumEMean$CALCIUM))
```
...
We calculate `p-value = 2.2e-16`, and thus __reject the null hypothesis__. The two periods have distinct means.
...
# Calculate the p-value
tost((dirbiliC %>% filter(regimen == 'control'))[,2], (dirbiliC %>% filter(regimen == 'MON810'))[,2], alpha = 0.05, epsilon = sd(dirbiliEMean$DIRBILI))
```
...
questionA_ttest <- function(variable, alpha, m){
  ...
  if(dim((dataNew %>% filter(sex == 'M')))[1] > 0 && dim(dataNew %>% filter(sex == 'F'))[1] > 0){
    ...
    # Perform the t-test
    return(t.test((dataNew %>% filter(sex == 'M'))[,2], (dataNew %>% filter(sex == 'F'))[,2], conf.level = 1 - alpha, var.equal = varEqual)$p.val/m)
  }
  else{
    return(NA)
  }
...
questionA_wilcox <- function(variable, alpha, m){
  ...
  if(dim((dataNew %>% filter(sex == 'M')))[1] > 0 && dim(dataNew %>% filter(sex == 'F'))[1] > 0){
    # Wilcox p-value
    return(wilcox.test((dataNew %>% filter(sex == 'M'))[,2], (dataNew %>% filter(sex == 'F'))[,2], alternative = 'two.sided', conf.level=1-alpha)$p.val/m)
  }
  else{
    return(NA)
  }
...
questionB_ttest <- function(variable, alpha, m){
  ...
  if(dim(dataNew %>% filter(period == 2))[1] > 0 && dim(dataNew %>% filter(period == 1))[1] > 0){
    ...
    # Perform the t-test
    return(t.test((dataNew %>% filter(period == 1))[,2], (dataNew %>% filter(period == 2))[,2], conf.level = 1 - alpha, var.equal = varEqual)$p.value/m)
  }
  else{
    return(NA)
  }
...
questionB_wilcox <- function(variable, alpha, m){
  ...
  if(dim(dataNew %>% filter(period == 2))[1] > 0 && dim(dataNew %>% filter(period == 1))[1] > 0){
    # Wilcox p-value
    return(wilcox.test((dataNew %>% filter(period == 1))[,2], (dataNew %>% filter(period == 2))[,2], alternative = 'two.sided', conf.level=1-alpha)$p.val/m)
  }
  else{
    return(NA)
  }
...
questionC_ttest <- function(variable, alpha, m){
  ...
  if(dim((dataNew %>% filter(regimen == 'control')))[1] > 0 && dim(dataNew %>% filter(regimen == 'MON810'))[1] > 0){
    ...
    # Perform the t-test
    return(t.test((dataNew %>% filter(regimen == 'control'))[,2], (dataNew %>% filter(regimen == 'MON810'))[,2], conf.level = 1 - alpha, var.equal = varEqual)$p.val/m)
  }
  else{
    return(NA)
  }
...
questionC_wilcox <- function(variable, alpha, m){
  ...
  if(dim((dataNew %>% filter(regimen == 'control')))[1] > 0 && dim(dataNew %>% filter(regimen == 'MON810'))[1] > 0){
    # Wilcox p-value
    return(wilcox.test((dataNew %>% filter(regimen == 'control'))[,2], (dataNew %>% filter(regimen == 'MON810'))[,2], alternative = 'two.sided', conf.level=1-alpha)$p.val/m)
  }
  else{
    return(NA)
  }
...
Bonferroni's correction derives from that fact that the probability of identifying at least one significant result due to chance increases as more hypotheses are tested. Therefore we divide each p-value by the number of tests being done per question, in this case we calculated `60`, resulting in a new results table:
```{r}
wilcoxBonferroni <- data.frame()
ttestBonferroni <- data.frame()

for (i in 1:length(variable)){
  # Wilcox results
  wilcoxBonferroni <- rbind(wilcoxBonferroni, cbind(variable[i], questionA_wilcox(variable[i], 0.05, 60-sum(is.na(wilcox$a))), questionB_wilcox(variable[i], 0.05, 60-sum(is.na(wilcox$b))), questionC_wilcox(variable[i], 0.05, 60-sum(is.na(wilcox$c)))))
  # t-test results
  ttestBonferroni <- rbind(ttestBonferroni, cbind(variable[i], questionA_ttest(variable[i], 0.05, 60-sum(is.na(ttest$a))), questionB_ttest(variable[i], 0.05, 60-sum(is.na(ttest$b))), questionC_ttest(variable[i], 0.05, 60-sum(is.na(ttest$c)))))
}

colnames(wilcoxBonferroni) <- c('variable', 'a Bonferroni', 'b Bonferroni', 'c Bonferroni')
colnames(wilcoxBonferroni) <- c('variable', 'a Bonferroni', 'b Bonferroni', 'c Bonferroni')
...
```{r}
bonWilcox05 <- as.data.frame(cbind(variable, bonWilcox[,2:4] < 0.05))
bonWilcox05
```
